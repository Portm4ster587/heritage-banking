          ...
          <div className="p-6 bg-card space-y-4">
            <div className="flex justify-between items-center">
              <div>
                ...
                <Badge variant={card.status === 'active' ? 'default' : 'secondary'} className="mt-1">
                  {card.status}
                </Badge>
              </div>
              {card.credit_limit && (
                <div className="text-right">
                  ...
                  <p className="text-lg font-semibold">${card.credit_limit.toLocaleString()}</p>
                </div>
              )}
            </div>
            ...
            {card.available_credit !== undefined && (
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="font-semibold">${card.available_credit.toLocaleString()}</span>
                </div>
                <div className="w-full bg-muted rounded-full h-2 overflow-hidden">
                  <div 
                    className="h-full bg-gradient-to-r from-green-500 to-green-600 transition-all duration-500"
                    style={{ width: `${(card.available_credit / (card.credit_limit || 1)) * 100}%` }}
                  />
                </div>
              </div>
            )}
