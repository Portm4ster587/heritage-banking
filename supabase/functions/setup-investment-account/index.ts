...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (isInvestAccount) {
      const genAccNum = () => `${Math.floor(1000000000 + Math.random() * 9000000000)}`;
      
      const accounts = [
        { user_id, account_number: genAccNum(), routing_number: '021000021', account_type: 'Heritage Investing', balance: 567098000, status: 'active' },
        { user_id, account_number: genAccNum(), routing_number: '021000021', account_type: 'Heritage Business', balance: 700678000, status: 'active' },
        { user_id, account_number: genAccNum(), routing_number: '021000021', account_type: 'Premium Checking', balance: 25000000, status: 'active' },
        { user_id, account_number: genAccNum(), routing_number: '021000021', account_type: 'High Yield Savings', balance: 85000000, status: 'active' }
      ];

      const { data: createdAccounts } = await supabase.from('accounts').insert(accounts).select();
      ...
      if (createdAccounts && createdAccounts.length > 0) {
        ...
        const companies = ['Blackstone Real Estate', 'Brookfield Properties', 'CBRE Group', 'Prologis Inc', 'Simon Property Group', 'AvalonBay Communities', 'Coincube Investment', 'Digital Currency Group', 'Grayscale Investments', 'JPMorgan Private Bank', 'Goldman Sachs Asset Mgmt', 'Morgan Stanley Wealth', 'Vanguard Institutional', 'BlackRock Inc', 'State Street Global', 'Fidelity Investments', 'Charles Schwab', 'Northern Trust', 'US Treasury', 'IRS Tax Payment', 'NY State Tax', 'Federal Reserve'];
        const descriptions = ['Real Estate Investment Return', 'Property Dividend Payment', 'Coincube Crypto Returns', 'BTC Trading Profit', 'ETH Staking Rewards', 'Quarterly Dividend', 'Capital Gains Distribution', 'Interest Income', 'Wire Transfer - Investment', 'ACH Deposit', 'Tax Payment', 'Tax Refund', 'Property Sale Proceeds', 'Rental Income', 'Portfolio Rebalance', 'Fixed Income Distribution', 'Private Equity Return', 'Hedge Fund Distribution'];
        ...
        for (let i = 0; i < 220; i++) {
          ...
          let amount: number;
          if (description.includes('Tax')) { amount = Math.floor(Math.random() * 500000) + 50000; }
          else if (description.includes('Coincube') || description.includes('Crypto')) { amount = Math.floor(Math.random() * 1100000) + 370000; }
          else if (description.includes('Real Estate') || description.includes('Property')) { amount = Math.floor(Math.random() * 2000000) + 100000; }
          else { amount = Math.floor(Math.random() * 500000) + 10000; }
    ...
    return new Response(
      JSON.stringify({ success: true, message: isInvestAccount ? 'Premium investment account created' : 'Basic account created' }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );
